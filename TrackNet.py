...
class TrackNet(nn.Module):
    ...
    def _make_convolution_layer(self, in_channels, out_channels, num, dropout_rate=0.0):
        layers = []
        layers.append(
            self._make_convolution_sublayer(
                in_channels, out_channels, dropout_rate=dropout_rate
            )
        )
        for _ in range(num - 1):
            layers.append(
                self._make_convolution_sublayer(
                    out_channels, out_channels, dropout_rate=dropout_rate
                )
            )
    ...
    def __init__(self, opt):
        ...
        # VGG16
        if opt.grayscale:
            self.vgg_conv1 = self._make_convolution_layer(
                opt.sequence_length, 64, 2, dropout_rate=opt.dropout
            )
        else:
            self.vgg_conv1 = self._make_convolution_layer(
                3 * opt.sequence_length, 64, 2, dropout_rate=opt.dropout
            )
        self.vgg_maxpool1 = nn.MaxPool2d((2, 2), stride=(2, 2))
        self.vgg_conv2 = self._make_convolution_layer(
            64, 128, 2, dropout_rate=opt.dropout
        )
        self.vgg_maxpool2 = nn.MaxPool2d((2, 2), stride=(2, 2))
        self.vgg_conv3 = self._make_convolution_layer(
            128, 256, 3, dropout_rate=opt.dropout
        )
        self.vgg_maxpool3 = nn.MaxPool2d((2, 2), stride=(2, 2))
        self.vgg_conv4 = self._make_convolution_layer(
            256, 512, 3, dropout_rate=opt.dropout
        )

        # Deconv / UNet
        self.unet_upsample1 = nn.UpsamplingNearest2d(scale_factor=2)
        self.unet_conv1 = self._make_convolution_layer(
            768, 256, 3, dropout_rate=opt.dropout
        )
        self.unet_upsample2 = nn.UpsamplingNearest2d(scale_factor=2)
        self.unet_conv2 = self._make_convolution_layer(
            384, 128, 2, dropout_rate=opt.dropout
        )
        self.unet_upsample3 = nn.UpsamplingNearest2d(scale_factor=2)
        self.unet_conv3 = self._make_convolution_layer(
            192, 64, 2, dropout_rate=opt.dropout
        )
        ...
        if opt.one_output_frame:
            ...
            self.last_conv = nn.Conv2d(
                64, opt.sequence_length, kernel_size=(1, 1), padding="same"
            )
